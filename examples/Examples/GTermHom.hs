...
type GHom q f g = forall a . (?below :: a -> q, ?above :: q) => f a -> Context g a
...
toUpTrans :: (Functor f, Functor g) => UpState f q -> GHom q f g -> UpTrans q f g
...
upHom :: (Functor f, Functor g) => UpState f q -> GHom q f g -> Term f -> (q,Term g)
...
toDownTrans :: Zippable f => DownState f q -> GHom q f g -> DownTrans q f g
...
downHom :: (Zippable f, Functor g)
            => DownState f q -> GHom q f g -> q -> Term f -> Term g
